(function(){
	...
	function setTag(){
		if(tagContent.children){
			for(var i=0;i<tagContent.children.length;i++){
				tagContent.children[i].className="tagItem content";
				var tagItem=document.getElementsByClassName("tagItem");
				for(var j=0;j<tagItem.length;j++){
					eventUtil.addEvent(tagItem[j],"mouseover",function(){
						this.innerHTML="删除"+this.innerHTML;
						eventUtil.addEvent(this,"click",function(){
							var index=queue.arr2.indexOf(this.innerHTML.slice(2));
							queue.arr2.splice(index,1);
							queue.paint(tagContent,queue.arr2);
							setTag();
						})
					});
					eventUtil.addEvent(tagItem[j],"mouseout",function(){
